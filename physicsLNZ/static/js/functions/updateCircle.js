function updateCircle(id) {
	...
	var material = $(`#circle-material-${id}`).val();

	var newCenterX = bodyToUpdate.GetPosition().x;
	var newCenterY = bodyToUpdate.GetPosition().y;
	if (material == "none"){
		newDensity = Number($(`#circle_density-${id}`).val());
		newFriction = Number($(`#circle_friction-${id}`).val());
		newRestitution = Number($(`#circle_restitution-${id}`).val());	
		if (newDensity < 0) {
			newDensity = 0;
		}
		if (newFriction < 0) {
